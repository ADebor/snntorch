...
@hydra.main(
    config_path="config/", config_name="config_mujoco", version_base="1.1"
)
def main(cfg: "DictConfig"):  # noqa: F821
    ...
    num_mini_batches = (
        cfg.collector.frames_per_batch // cfg.loss.mini_batch_size
    )
    ...
    collector = SyncDataCollector(
        create_env_fn=make_parallel_env(
            cfg.env.env_name, cfg.collector.num_envs, device
        ),
        ...
        compile_policy=(
            {"mode": compile_mode, "warmup": 1} if compile_mode else False
        ),
    ...
    if cfg.compile.compile:
        ...
        adv_module = compile_with_warmup(
            adv_module, mode=compile_mode, warmup=1
        )
